//! Provider and model configuration for the agent layer.
//!
//! Holds the model id triplet (`primary` / `small` / `reasoning`), the
//! Anthropic-style beta-flag set, and per-provider model defaults. The
...
use crate::agent::types::{DEFAULT_MAX_TOKENS, MIN_THINKING_BUDGET};
...
/// Top-level provider family the agent runtime targets.
///
