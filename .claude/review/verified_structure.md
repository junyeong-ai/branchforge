...
- `src/session/tool_state.rs:60-125` — `ToolState(Arc<ToolStateInner>)` wraps `{session: RwLock, executions, input_queue, execution_lock, executing, queue_notify}`. Single-session concurrent handle, **not** a map keyed by dynamic ids.
...
| D12 | `src/session/tool_state.rs:125` | `ToolState` name hides concurrent session handle role | P2-1 → `SessionHandle` |
