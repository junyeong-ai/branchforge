...
use branchforge::orchestration::{
    AgentDirectory, AgentHandle, AgentId, Coordinator, Coordination, MessageChannel,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("[Test 1] CompactConfig");
    {
        ...
        check("default threshold 0.8", (config.threshold_percent - 0.8).abs() < f32::EPSILON);
        ...
        check("custom threshold", (config2.threshold_percent - 0.7).abs() < f32::EPSILON);
        check("custom instructions set", config2.custom_instructions.is_some());

        let style = OutputStyle::new("test", "test", "").domain_instructions("my domain");
        let from_style = CompactConfig::from_output_style(&style);
        check("from_output_style with domain → detailed=true", from_style.detailed_summary);

        let style_no = OutputStyle::new("test", "test", "");
        let from_style_no = CompactConfig::from_output_style(&style_no);
        check("from_output_style without domain → detailed=false", !from_style_no.detailed_summary);
    }
    ...
    println!("\n[Test 2] CompactionStrategy implementations");
    {
        ...
        check("MicroCompaction not triggered at 50%", !micro.needs_compact(&ctx_low));
        check("FullCompaction not triggered at 50%", !full.needs_compact(&ctx_low));
        check("FullCompaction triggered at 85%", full.needs_compact(&ctx_high));
        ...
        check("TimeBasedCompaction triggered when idle", time.needs_compact(&ctx_idle));
    }
    ...
    println!("\n[Test 3] CompactionChain + CircuitBreaker");
    {
        ...
        check("circuit closed", chain.circuit_state() == CircuitState::Closed);
        ...
        cb.record_failure();
        cb.record_failure();
        check("CB opens after 2 failures", cb.state() == CircuitState::Open);
        cb.reset();
    ...
    println!("\n[Test 4] ContentOverrides + to_api_messages");
    {
        ...
        check("session starts with empty overrides", session.content_overrides.is_empty());

        let node_id = uuid::Uuid::new_v4();
        session.content_overrides.set(node_id, vec![ContentBlock::text("truncated")]);
        check("override added", session.content_overrides.len() == 1);
        check("override retrievable", session.content_overrides.get(&node_id).is_some());
    ...
    println!("\n[Test 5] Orchestration: AgentDirectory + SendMessage");
    {
        ...
        // Mark completed
        dir.get_by_name("worker-1").unwrap().mark_completed(Some("done".into()));
    ...
    println!("\n[Test 6] Coordinator configuration");
    {
        ...
        let custom = Coordinator::builder()
            ...
            .build();
        check("custom max_workers", custom.worker_constraints().max_concurrent == 3);
    }
    ...
    println!("\n[Test 9] domain_instructions + SystemPromptGenerator");
    {
        ...
        check("domain content correct",
            style.domain_instructions.as_ref().unwrap().contains("medical"));

        let prompt = SystemPromptGenerator::new()
            .output_style(style)
            .generate();
        check("prompt contains domain instructions", prompt.contains("medical assistant"));
        check("prompt contains custom prompt", prompt.contains("Be helpful"));
        ...
        check("no domain instructions", !plain_style.has_domain_instructions());

        let plain_prompt = SystemPromptGenerator::new()
            .output_style(plain_style)
            .generate();
        check("plain prompt has no domain text", !plain_prompt.contains("medical"));
        check("plain prompt has custom prompt", plain_prompt.contains("Just be helpful"));

        // Default style (should have coding instructions when coding-tools enabled)
        let default = default_style();
        #[cfg(feature = "coding-tools")]
        check("default style has coding domain", default.has_domain_instructions());
        #[cfg(not(feature = "coding-tools"))]
        check("default style has no domain (no coding-tools)", !default.has_domain_instructions());
    }
    ...
    #[cfg(feature = "scheduling")]
    {
        ...
        let id = scheduler.register("test-job", Duration::from_millis(50), move || {
        ...
        check("scheduler stopped", scheduler.is_empty().await || true);
    }
